...
// stakecoin: optional setting to unlock wallet for block minting only;
...
int64 CWalletTx::GetTxTime() const
{
    ...
    //stakecoin: we still have the timestamp, so use it to avoid confusion
    return nTime;
}
...
// stakecoin: total coins staked (non-spendable until maturity)
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            if (pcoin->nTime > nSpendTime)
                continue;  // stakecoin: timestamp must not exceed spend time
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            loop
            {
                ...
                if (nChange > 0)
                {
                    ...
                    else if (!GetBoolArg("-avatar")) // stakecoin: not avatar mode; nu: avatar mode enabled by default only on Share wallet to avoid change being sent to hidden address
                    {
...
// stakecoin: check 'spent' consistency between wallet and txindex
// stakecoin: fix wallet spent state according to txindex
...
// stakecoin: disable transaction (only for coinstake)
