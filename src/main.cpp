...
// Copyright (c) 2011-2015 The stakecoin developers
...
void SyncWithWallets(const uint256 &hash, const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // stakecoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
...
bool CCoinsView::GetUtxos(const std::string & address, std::set<CCoins> & coinset) { return false; }
...
bool CCoinsViewBacked::GetUtxos(const std::string & address, std::set<CCoins> & coinset) { return base->GetUtxos(address, coinset);}
...
bool CTransaction::CheckTransaction(CValidationState &state) const
{
    ...
    BOOST_FOREACH(const CTxOut& txout, vout)
    {
        ...
        // stakecoin: enforce minimum output amount
                        ...
                        bool* pfMissingInputs)
{
    ...
    // stakecoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
int64 GetProofOfWorkReward(unsigned int nBits)
{
    ...
    // stakecoin: subsidy is cut in half every 16x multiply of difficulty
...
// stakecoin: miner's coin stake is rewarded based on coin age spent (coin-days)
...
// stakecoin: find last block index up to pindex
...
unsigned int static GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // stakecoin: target change every block
    // stakecoin: retarget with exponential moving toward target spacing
    CBigNum bnNew;
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    // stakecoin: should not enter safe mode for longer invalid chain
}
...
bool CTransaction::CheckInputs(CValidationState &state, CCoinsViewCache &inputs, bool fScriptChecks, unsigned int flags, std::vector<CScriptCheck> *pvChecks) const
{
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // stakecoin: check transaction timestamp
        ...
        if (IsCoinStake())
        {
            // stakecoin: coin stake tx earns reward instead of paying fee
            uint64 nCoinAge;
        ...
        else
        {
            ...
            // stakecoin: enforce transaction fees for every block
            if (nTxFee < GetMinFee())
...
bool CBlock::DisconnectBlock(CValidationState &state, CBlockIndex *pindex, CCoinsViewCache &view, bool *pfClean)
{
        ...
        if (i > 0) { // not coinbases
            ...
            for (unsigned int j = tx.vin.size(); j-- > 0;) {
                ...
                if (undo.nHeight != 0) {
                    ...
                    coins.fCoinStake = undo.fCoinStake; // stakecoin
                    coins.nTime = undo.nTime;           // stakecoin
                } else {
                    if (coins.IsPruned())
    ...
    // stakecoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CValidationState &state, CBlockIndex* pindex, CCoinsViewCache &view, bool fJustCheck)
{
    ...
    // stakecoin: coinbase reward check relocated to CheckBlock()
    ...
    // stakecoin: track money supply and mint amount info
    pindex->nMint = nValueOut - nValueIn + nFees;
    pindex->nMoneySupply = (pindex->pprev? pindex->pprev->nMoneySupply : 0) + nValueOut - nValueIn;

    // stakecoin: fees are not collected by miners as in bitcoin
    // stakecoin: fees are destroyed to compensate the entire network
    if (fDebug && GetBoolArg("-printcreation"))
...
// stakecoin: total coin age spent in transaction, in the unit of coin-days.
...
// stakecoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(CValidationState &state, const CDiskBlockPos &pos)
{
    ...
    // stakecoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // stakecoin: record proof-of-stake hash value
    ...
    // stakecoin: compute stake modifier
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // stakecoin: only the second transaction can be the optional coinstake
    for (unsigned int i = 2; i < vtx.size(); i++)
        if (vtx[i].IsCoinStake())
            return state.DoS(100, error("CheckBlock() : coinstake in wrong position"));

    // stakecoin: coinbase output should be empty if proof-of-stake block
    ...
    // stakecoin: check block signature
...
bool CBlock::AcceptBlock(CValidationState &state, CDiskBlockPos *dbp)
{
    ...
    if (hash != hashGenesisBlock) {
        ...
        // stakecoin: check that the block satisfies synchronized checkpoint
    ...
    // stakecoin: check pending sync-checkpoint
...
bool ProcessBlock(CValidationState &state, CNode* pfrom, CBlock* pblock, CDiskBlockPos *dbp)
{
    ...
    // stakecoin: check proof-of-stake
    ...
    // stakecoin: verify hash target and signature of coinstake tx
    if (pblock->IsProofOfStake())
    {
    ...
    // stakecoin: ask for pending sync-checkpoint if any
    ...
    if (pblock->hashPrevBlock != 0 && !mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom) {
            ...
            // stakecoin: getblocks may not obtain the ancestor block rejected
    ...
    // stakecoin: if responsible for sync-checkpoint send it
...
// stakecoin: entropy bit for stake modifier if chosen by modifier
...
bool static LoadBlockIndexDB()
{
    ...
    BOOST_FOREACH(const PAIRTYPE(int, CBlockIndex*)& item, vSortedByHeight)
    {
        ...
        // stakecoin: calculate stake modifier checksum
    ...
    // stakecoin: load hashSyncCheckpoint
...
bool LoadBlockIndex()
{
    ...
    if (fTestNet)
    {
        ...
        bnInitialHashTarget = CBigNum(~uint256(0) >> 1);
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n"
            , fTestNet? "Test" : "stakecoin"
            , hashGenesisBlock.ToString().substr(0, 20).c_str()
            , bnProofOfWorkLimit.GetCompact()
            , bnInitialHashTarget.GetCompact()
            , nStakeMinAge
            , nCoinbaseMaturity
            , nModifierInterval);
...
bool InitBlockIndex() {
    ...
    fTxIndex = true; // stakecoin: txindex is always enabled
    ...
    if (!fReindex) {
        // Genesis block
        const char* pszTimestamp = "Shanghai index closed at 3129.85, 9th Feb., 2018";
        CTransaction txNew;
        txNew.nTime = 1518351226;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        // stakecoin: initialize synchronized checkpoint
        if (!WriteSyncCheckpoint(hashGenesisBlock))
            return error("LoadBlockIndex() : failed to init sync checkpoint");
    }

    // stakecoin: if checkpoint master key changed must reset sync-checkpoint
...
string GetWarnings(string strFor)
{
    ...
    // stakecoin: wallet lock warning for minting
    if (strMintWarning != "")
    {
        nPriority = 0;
    ...
    // stakecoin: if detected invalid checkpoint enter safe mode
    ...
    // Alerts
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CAlert)& item, mapAlerts)
        {
            ...
            if (alert.AppliesToMe() && alert.nPriority > nPriority)
            {
                ...
                if (nPriority > 1000)
                    strRPC = strStatusBar;  // stakecoin: safe mode for high alert
            }
        }
    }
...
void static ProcessGetData(CNode* pfrom)
{
    ...
    while (it != pfrom->vRecvGetData.end()) {
        ...
        const CInv &inv = *it;
        {
            ...
            // stakecoin: relay memory may contain blocks too
            bool found = false;
            ...
            if (inv.type == MSG_BLOCK || inv.type == MSG_FILTERED_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    if (inv.hash == pfrom->hashContinue)
                    {
                        ...
                        // stakecoin: send latest proof-of-work block to allow the
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // stakecoin: record my external IP reported by peer
        ...
        // stakecoin: relay sync-checkpoint
        {
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // stakecoin: tell downloading node about the latest block if it's
...
CBlockTemplate* CreateNewBlock(CReserveKey& reservekey, CWallet* pwallet, bool fProofOfStake)
{
    ...
    // stakecoin: if coinstake available add coinstake tx
    ...
    int64 nFees = 0;
    {
        ...
        while (!vecPriority.empty())
        {
            ...
            // stakecoin: simplify transaction fee - allow free = false
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("stakecoinMiner : proof-of-work not meeting target");

    //// debug print
    printf("stakecoinMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("stakecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("stakecoinMiner : ProcessBlock, block not accepted");
    }
...
void BitcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    RenameThread(fProofOfStake? "stakecoin-stake-minter" : "stakecoin-miner");
    ...
    try { loop {
        ...
        if (fProofOfStake)
        {
            // stakecoin: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake())
            {
        ...
        printf("Running stakecoinMiner with %" PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        if(!fProofOfStake) printf("stakecoinMiner terminated\n");
        throw;
    }
}
...
// stakecoin: stake minter
void MintStake(boost::thread_group& threadGroup, CWallet* pwallet)
{
    // stakecoin: mint proof-of-stake blocks in the background
